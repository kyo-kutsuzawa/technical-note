...
このように、加速度制御に基づいたインピーダンス制御系の設計を通して、所望の運動特性をもつバイラテラル制御系が設計できる。
さらに制御器を適切な座標系に座標変換することで、機能的意味が明快であり見通しのよい設計が可能になる。

なお、モード座標系では制御インピーダンスが無限もしくはゼロになるように設計したものの、実際にはゲインを無限大にすることは不可能であり、加えて制御系の物理的解釈や設計の一般性も考えると、**上述のバイラテラル制御はインピーダンス制御の一種として説明することが有用である**と考えられる。
